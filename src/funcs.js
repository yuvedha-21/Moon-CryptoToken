...
export const load = async () => {
  ...
  const { ethFunds, transactionCount, tokensSold, ethPriceN, transactions } =
    await loadVariables(contractFTS);
...
const loadVariables = async (contractFTS) => {
  const admin = "0xE1a0E1DbDC7498eacdAEcDcAF06eA423ae68721E";
...
const loadContracts = async () => {
  const FTContract = contract(Token);
  FTContract.setProvider(window.web3.currentProvider);
  const FTSContract = contract(TokenSale);
